...
class Solution
{
  public:
    string canBeTwoPowerSum(int num)
    {
        ...
        return to_string(power1 + power2) + " = 2^" + to_string(bit1) +
               " + 2^" + to_string(bit2);
    }
};
...
int main()
{
    ...
    cout << "Can n = " << n1 << " be split by two power sum? => "
         << solution.canBeTwoPowerSum(n1) << endl;

    int n2 = 10;
    cout << "Can n = " << n2 << " be split by two power sum? => "
         << solution.canBeTwoPowerSum(n2) << endl;

    int n3 = 7;
    cout << "Can n = " << n3 << " be split by two power sum? => "
         << solution.canBeTwoPowerSum(n3) << endl;

    int n4 = 8;
    cout << "Can n = " << n4 << " be split by two power sum? => "
         << solution.canBeTwoPowerSum(n4) << endl;

    int n5 = -8;
    cout << "Can n = " << n5 << " be split by two power sum? => "
         << solution.canBeTwoPowerSum(n5) << endl;
